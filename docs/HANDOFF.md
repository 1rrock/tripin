...
- **커밋 안 된 변경 75개.** 이번 세션 작업 전체가 워킹 트리에만 있다 — §2-3 을 먼저 읽어라.

---

## 1. 이번 세션에 바뀐 것 (커밋 없음 — 전부 워킹 트리)

`git log` 최신은 여전히 `4cbff9e` 다. 아래는 **전부 미커밋**이다.
...
병렬 에이전트가 만든 것들이다. 이 목록 자체가 §3-5 의 근거다.
...
### 🟡 3. 커밋 — 75개 파일이 워킹 트리에만 있다

이번 세션 작업 **전체**가 미커밋이다. 기본 브랜치는 `main` 이 아니라 **`master`**(원격 없음).
한 덩어리로 커밋하면 나중에 이분 탐색이 불가능하니, §1 의 구분(P0 / P1 / 회귀 수정 / 푸터 /
EN 번역)대로 쪼개는 걸 권한다.
...
**확인법**: 화면 텍스트가 아니라 **응답 전체**를 grep 한다 — 0 이어야 한다.
```bash
curl -s localhost:3000/en/c/chuseonghoon/tokyo | grep -c "돈카츠 전문점"
```

### 3-3. ⚠️ 낡은 서버가 포트를 잡고 있으면 **옛 빌드를 측정하게 된다**
...
### 3-4. ⚠️ `.env.local` 은 **키 이름이 아니라 값**을 확인하라
...
### 3-5. ⚠️ 병렬 에이전트는 **같은 파일을 덮어쓰고, 서로의 의도를 오해한다**
...
### 3-6. ⚠️ "가장 가까운 기존 키를 재사용하라"는 지시가 **카피를 죽인다**
