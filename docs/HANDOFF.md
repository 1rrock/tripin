...
- **봉인된 컴포넌트** — `shared/ui/frame.tsx` 의 `Chip`, `shared/ui/Button.tsx`,
  ...
  button 갈래까지 들여 **봉인 밖 예외가 0개**가 됐다. row 갈래는 단추가 아니라
  목록 행(`ROW_BODY`)이라 그대로 둔다.
...
| A-7 | **`/saved` 데스크톱 "그룹 만들기" 단추** — `Button`(secondary·md)으로 이관했다. 라운드가 10px → 12px 로 2px 커진 것 말고 달라 보이면 안 된다 |
| A-8 | **`/en` 상호명 표본 검수** — 기계 표기라 같은 체인의 지점명이 배치 경계에서 갈릴 수 있다(실제로 "Gyoza no Katchan / Kat-chan / Kacchan" 세 표기가 나왔다). 눈에 걸리는 것만 어드민에서 고치면 된다 |

### 5-2. 결정이 필요한 것 — 사람 몫

| 항목 | 왜 |
|---|---|
| **`ADMIN_SECRET` 이 비밀번호이자 HMAC 서명 키** | 분리하면 기존 세션이 전부 끊긴다. **시크릿 회전 정책 결정이 필요하다.** (`docs/ADMIN.md` 1.5) |
| **`0020_drop_bulk_confirm_backup.sql` 미적용** | `_bulk_confirm_20260805` 12행을 정리하는 대가로 `0002` 롤백 절차의 근거를 잃는다. 그 대상에 재검토가 안 끝난 장소 여섯이 있다. ⚠️ `npm run db:migrate` 를 **인자 없이 돌리면 같이 적용된다** — 개별 실행(`db:run -- <파일>`)을 써라 |
| **ledger 오염 26행** | `_migrations` 44행 중 26행이 임시파일(`q.sql`·`q2~q15.sql`…). 재발은 막았지만 기존 행은 남아 있다. 정리 SQL 은 `scripts/clean-migration-ledger.sql`, **사람이 직접 돌린다.** 지우기 전에 같은 basename 의 정식 마이그레이션 계획이 없는지 확인 |
| **`summary_en` 0행** | `translate-en.mjs`(이름 모드 아님)를 **한 번도 안 돌렸다.** EN 요약은 §3-8 대로 숨겨진다 — 깨진 건 없지만 EN 페이지에 산문이 없다. 비용·범위 판단 필요 |
