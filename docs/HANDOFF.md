...
- 브랜드 `Tripin → Greatripin → Eatripin` — UI/워드마크/마크 코드 반영. 문서 일부(PRODUCT 등)는 아직 Tripin 가칭.
...
### ✅ 4-2. 구글 링크가 있으면 **주소가 모호해도 통과** (2026-08-10 결정)

판정 기준은 "주소 문자열이 예쁜가"가 아니라 **"유저가 찾아갈 수 있는가"** 다.
구글 링크가 있으면 핀은 좌표가, 길찾기는 `place_id`/공유 URL 이 결정하고 주소 문자열은
표시용일 뿐이다. `isVagueAddress` 가 **지도 링크를 먼저 보고 빠져나온다.**

결과: 공개 기준 **8건 → 0건**. 8곳 전부 좌표와 구글 링크가 있어 지도에서 정확히 찍히는
곳들이었다. 현재 133곳 **전부** 구글 링크가 있어 이 플래그는 당분간 0으로 남는다 —
고장이 아니라 "수집이 링크를 제대로 채우고 있다"는 신호다.

이 규칙을 두 번 틀렸다(§3 에 안 넣은 이유: 코드 주석에 이미 박아 뒀다):
`\(` 를 얼버무림으로 쳐서 지번 병기 주소 9건을 잡았고, 그다음엔 구글 링크가 있는데도
주소 문구만 보고 8건을 잡았다.

⚠️ 남은 것은 **표시 문구 품질**이지 찾기 문제가 아니다 — `오키나와 북부(나고 인근)
스테이크 전문` 은 주소라기보다 설명이다. 고칠 거면 `/admin/place/[id]` 에서 고친다.

### ✅ 5. 폐업 확인 — **완료. 4곳 비공개** (2026-08-10)

`npm run check:closed` 로 126곳을 훑었다(`google_place_id` 가 있는 전부).

| 상태 | 수 |
|---|---|
| OPERATIONAL | 122 |
| CLOSED_PERMANENTLY | **3** — 만우장 · 하나코지 사와다 · 야키니쿠 케이 |
| CLOSED_TEMPORARILY | **1** — 야구라 |
| UNKNOWN / NOT_FOUND | 0 |

**4곳 전부 비공개로 내렸다. 삭제하지 않았다** — 되돌릴 수 있게 두는 쪽으로 결정했다.

**삭제하지 않은 이유**(다시 논의될 때 이걸 먼저 봐라): 영구 폐업 3곳 중 **2곳은 영상이
그 장소만 가리킨다.** 게다가 그 영상들은 **제목이 곧 그 가게**다 —
`[sub] 성시경의 먹을텐데 l 부산 만우장`, `삿포로 여기어때 day 3. 미슐랭 3스타 하나코지 사와다`.
지우면 `video_places` 가 CASCADE 로 같이 지워져 **정거장 0개짜리 영상 2개**가 남는다.
비공개는 유저에게 안 보이는 효과가 같으면서 되돌릴 수 있다.

⚠️ **`확정했으나 비공개` 3건 = 영구 폐업 3곳이었다.** 그 3건을 공개로 올리려던 시도가
이번 세션의 출발점이었는데, 올렸으면 **폐업한 가게를 지도에 띄우는 것**이었다.
그 목록은 "올려야 할 것"이 아니라 **이미 판단이 끝난 것**일 수 있다 — 올리기 전에
`npm run check:closed` 를 먼저 돌려라.

⚠️ **`google_place_id` 가 없는 7곳은 확인 자체가 안 된다** — 텟판혼포 · 클라이맥스 커피
데포아일랜드 · 안동돼지국밥 덕포 · 돈카츠 요시다 · 스테이크맨 · 화국반점 ·
카메노이 호텔 닛코 유니시가와.

⚠️ **이 7곳도 `google_maps_url` 은 있다** — 유저는 지도로 찾아갈 수 있다. 없는 건
`place_id` 뿐이고, Places API 가 그걸 요구해서 **폐업 확인만** 안 되는 것이다.
`/admin/place/[id]` 의 구글 공유 링크 칸에 넣어 주면 다음 실행부터 자동으로 잡힌다.

```bash
npm run check:closed                              # 확인만
npm run check:closed -- --hide --with-temporary   # 폐업+휴업 비공개 (이번에 쓴 것)
```
결과는 `tmp-closed-report.json` 에도 남는다(gitignore). 다시 보려고 API 를 또 때리지 마라.

**주기적으로 돌려야 한다.** 가게는 계속 닫는다. 배포 후에는 크론에 얹는 걸 검토하라.
