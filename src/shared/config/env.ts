 ...
 * 환경 변수 wrapper.
 *
 * Greatripin은 **서버 런타임이 있다.**
 * (히든스팟은 앱인토스라 서버가 없어서 전부 클라이언트에서 Supabase를 직접 불렀다.)
 *
 * ⚠️ 공개 페이지는 SSG/ISR 이 아니다 — 로케일이 proxy 헤더로만 오는 구조라
 *    레이아웃이 `headers()` 를 읽고, 그래서 전부 요청마다 서버에서 렌더된다.
 *    비용은 렌더가 아니라 DB 왕복에 있고 그건 `shared/api/cache.ts` 가 캐시한다.
 *
 * 그래서 값이 두 종류로 갈린다:
