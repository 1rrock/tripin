 ...
 *
 * 목록의 한 줄짜리 링크는 첫 번째(primary)만, 상세 드로어와 어드민은 전부 보여준다.
 *
 * 순서는 나라에 따라 다르다 — 한국은 구글 지도로 도보·차량 길찾기가 막혀 있어
 * 사실상 못 쓴다. 한국(KR) 장소는 네이버 → 카카오 → 구글, 그 외는 구글 → 카카오 → 네이버.
 *
 * ⚠️ **ID 가 없는 앱을 이름 검색 링크로 채우지 마라.** 한때 드로어에서 구글·네이버를
 *    항상 고를 수 있게 하려고 그렇게 했는데, 네이버에 등록되지 않은 가게(해외가 대부분)는
 *    그 버튼이 빈 검색 결과나 엉뚱한 동명 가게로 열렸다. 고를 게 없는데 고르라고 내미는
 *    버튼이라, 없느니만 못하다. 딥링크를 만들 수 있는 앱만 내보낸다.
 *    (공개 장소 1806곳 중 ID 가 하나도 없는 곳은 0곳이라 아래 좌표 폴백은 실제로 안 쓰인다.)
 */
