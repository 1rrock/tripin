...
export default async function PlaceOpengraphImage({
  ...
  try {
    place = await loadPlaceBySlug(routeSlug(slug), "ko");
  } catch {
    place = null;
  }

  const m = getDictionary("ko");
  const name = place?.name ?? "여행 유튜버가 간 곳";
  const metaLine = place
    ? `${place.cityName} · ${m.placeTypes[place.placeType]}`
    : "";
          ...
          >
            {bold ? (
              ...
              <div style={{ display: "flex", flexDirection: "column", alignItems: "flex-start" }}>
                ...
                <div style={{ display: "flex", fontSize: 34, color: "#6e5c4a", marginTop: 28 }}>
          ...
          {thumb ? (
            <div
              style={{
                ...
                borderRadius: 16,
                overflow: "hidden",
                boxShadow: "0 0 0 1px rgba(42,33,24,0.12)",
              }}
            >
