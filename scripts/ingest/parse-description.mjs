...
const stripBullet = (s) =>
  s
    ...
    .replace(/\s+/g, " ")
    .trim()
    // "[대중술집 팔전 우메다 도야마점] 구글맵 링크" → "대중술집 팔전 우메다 도야마점"
    // 상호를 대괄호로 감싸고 뒤에 링크 라벨을 붙이는 포맷(우리초이). 그대로 두면
    // 대괄호와 "구글맵 링크" 가 카드 제목에 그대로 나간다.
    .replace(/^\[([^\]]+)\]\s*(?:구글\s?맵|구글\s?지도|네이버\s?지도|네이버|지도)?\s*(?:링크)?\s*$/, "$1")
    .trim();
