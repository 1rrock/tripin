...
| **ink / lemon** | 12.92 | **1.16** | ⚠️ **다크에서 파손** — 아래 참조 |

**The Fixed-Ink-On-Color Rule.** 코랄처럼 두 테마에서 같은 중간톤을 유지하는 색 위에서는 글자색도 **테마와 무관하게 고정**한다. 그래서 `--on-brand`(`#23120c`)가 존재한다. `--ink`를 쓰면 다크 테마에서 밝은 글자가 밝은 코랄 위에 얹혀 2.30:1로 무너진다. 코랄 위 **아이콘**(화살표 뱃지 등)은 비텍스트 요소라 3:1 기준을 통과하므로 반전색을 허용한다.

> ⚠️ **알려진 파손 — 레몬에는 이 규칙이 적용되지 않았다.** `--lemon`은 두 테마 모두 밝은 노랑(`#ffd43a` / `#ffd84d`)인데 짝이 되는 `--on-lemon` 고정값이 없어, 레몬 지면이 다크에서 `--ink`(`#e9ebee`)를 그대로 받아 **1.16:1**이 된다 — 글자가 보이지 않는다. 영향 표면: 홈 레몬 티커, 헤더 워드마크의 "in" 블록, 도시 칩·다음 행동 칩(6곳), `.hl-under` 히어로 밑줄. **코랄이 받은 처방(`--on-brand`)을 레몬에도 적용해야 한다.**
