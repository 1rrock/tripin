...
> 🚫 **폐기됨 (2026-08-10). 이 문서를 믿지 마라 — 코드와 정반대다.**
>
> 여기 적힌 "캐논"(웜 화이트 `#fffdf8` 지면 · 코랄 · 레몬 · 라이트/다크 2테마)은
> **삭제된 월드**다. 현재 월드는 **콘택트 시트(다크 전용)** — 암실 지면 `#0b0b0c`,
> 인화지 흰색 `#f5f3ef`, 왁스 연필 `#ff3d14`(표시 전용, 배경 금지), 라이트 테마 없음.
>
> 이 문서의 토큰 이름(`--paper`, `--card`, `--fill`, `--ink`, `--brand`, `--lemon` …)은
> **`globals.css` 에 존재하지 않는다.** `--paper` 는 여기선 지면인데 실제 코드에서는
> **글자색**이다 — 그대로 따라 쓰면 다크 지면에 다크 글자를 쓰게 된다.
>
> **진짜 디자인 계약이 있는 곳:**
> | 무엇 | 어디 |
> |---|---|
> | 방향 계약 (THESIS·OWN-WORLD·거부하는 배치) | `src/app/layout.tsx` 상단 HTML 주석 |
> | 토큰 전부 + 절대 하지 않는 것 | `src/app/globals.css` 상단 주석 |
> | 프리미티브 | `src/shared/ui/frame.tsx` |
>
> 남겨 둔 이유는 **결정 이력** 뿐이다(왜 Color Pop → 캐논으로 갔는지, 왜 두 월드로
> 갈라졌었는지). 새 화면을 만들 때 여기서 값을 가져오지 마라.
> 정식 재작성은 `docs/HANDOFF.md` 의 열린 항목에 있다.
